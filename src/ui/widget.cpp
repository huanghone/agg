...
void WidgeImp::create_pmap(unsigned width, unsigned height, rendering_buffer* wnd) {
...
static void convert_pmap(rendering_buffer* dst, const rendering_buffer* src,  pix_format_e format) {
	switch(format) {
	case pix_format_gray8:
		break;
	case pix_format_gray16:
...
bool WidgeImp::save_pmap(const char* fn, unsigned idx, const rendering_buffer* src) {
	if(m_sys_format == m_format) {
...
bool WidgeImp::load_pmap(const char* fn, unsigned idx, rendering_buffer* dst) {
	Pixelmap pmap_tmp;
	...
	if (m_specific->m_last_translated_key) {
		...
		if (window_flags() & window_process_all_keys) {
			...
			if (root_view_.on_arrow_keys(left, right, down, up)) {
				...
				on_key(m_specific->m_cur_x, m_specific->m_cur_y, m_specific->m_last_translated_key, m_specific->m_input_flags);
			}
		}
	}
}
...
LRESULT CALLBACK window_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_ERASEBKGND:
		break;
  case WM_LBUTTONDOWN:
		app->OnLButtonDown(LOWORD(lParam), HIWORD(lParam), wParam);
    break;
  case WM_LBUTTONUP:
		app->OnLButtonUp(LOWORD(lParam), HIWORD(lParam), wParam);
    break;
  case WM_RBUTTONDOWN:
		app->OnRButtonDown(LOWORD(lParam), HIWORD(lParam), wParam);
		break;
  case WM_RBUTTONUP:
		app->OnRButtonUp(LOWORD(lParam), HIWORD(lParam), wParam);
    break;
  case WM_MOUSEMOVE:
...
bool Widget::init(unsigned width, unsigned height, unsigned flags) {
  ...
  m_specific->m_hwnd = ::CreateWindow("AGGAppClass", m_caption, wflags, 100, 100, width, height, 0, 0, g_windows_instance, 0);

...
namespace agg {
RBoxCtrlBase::RBoxCtrlBase(double x1, double y1, double x2, double y2, bool flip_y) :
...
void RBoxCtrlBase::add_item(const char* text) {
	if(m_num_items < 32) {
...
void RBoxCtrlBase::border_width(double t, double extra) {
  m_border_width = t; 
  m_border_extra = extra;
  calc_rbox(); 
}

void RBoxCtrlBase::text_size(double h, double w)  {
  m_text_width = w; 
  m_text_height = h; 
}

void RBoxCtrlBase::rewind(unsigned idx) {
  m_idx = idx;
...
unsigned RBoxCtrlBase::vertex(double* x, double* y) {
...
bool RBoxCtrlBase::OnMouseButtonDown(double x, double y) {
  inverse_transform_xy(&x, &y);
  unsigned i;
...
bool RBoxCtrlBase::OnMouseMove(double, double, bool) {
  return false;
}

bool RBoxCtrlBase::OnMouseButtonUp(double, double) {
  return false;
}

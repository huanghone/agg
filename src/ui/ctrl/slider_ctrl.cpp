...
namespace agg {
SliderCtrlBase::SliderCtrlBase(double x1, double y1, double x2, double y2, bool flip_y) :
...
void SliderCtrlBase::rewind(unsigned idx) {
	...
	case 3:                 // pointer preview
		m_ellipse.init(
			m_xs1 + (m_xs2 - m_xs1) * m_preview_value, (m_ys1 + m_ys2) / 2.0,
			m_y2 - m_y1, m_y2 - m_y1,
			32);
		break;
	...
	case 4:                 // pointer
		...
		m_ellipse.init(
			m_xs1 + (m_xs2 - m_xs1) * m_value, (m_ys1 + m_ys2) / 2.0,
			m_y2 - m_y1,
			m_y2 - m_y1, 
			32);
		m_ellipse.rewind(0);
